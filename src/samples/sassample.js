...
function createBlob() {
  ...
  blobService.createBlockBlobFromText(container, blob, 'test blob', function (error, result, reponse) {
    if (error) {
      console.log(error);
    } else {
      console.log('Created the blob %s', container);
...
function downloadBlobUsingSharedAccessSignature() {
  ...
  var expiryDate = new Date(startDate.valueOf());
  ...
  sharedBlobService.getBlobProperties(container, blob, function (error, result) {
    if (error) {
      ...
      console.log('Downloaded the blob %s by using the shared access signature URL %s', blob, signature.url());
    }
...
function createPolicies() {
  ...
  var readWriteExpiryDate = new Date(readWriteStartDate.valueOf());
  ...
  blobService.setContainerAcl(container, BlobConstants.BlobContainerPublicAccessType.CONTAINER, options, function (error, result) {
    if (error) {
      console.log(error);
    } else {
      console.log('Uploaded the permissions for the container %s', container);
...
function usePermissions(readWriteSharedAccessPolicy, readSharedAccessPolicy) {
  ...
  sharedBlobService.getBlobProperties(container, blob, function (error, result) {
    if (error) {
      ...
      console.log('Downloaded the blob %s by using the shared access signature URL %s', blob, sharedAccessSignature.url());
    }
