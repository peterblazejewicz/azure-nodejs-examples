...
function uploadblockBlobs() {
  ...
  blobClient.createBlockFromText(blockList.LatestBlocks[0], container, blob, blockContent1, blockContent1.length, function (error, response) {
    if (error) {
      console.log(error);
    } else {
      console.log('Uploaded the block whose ID is %s', blockList.LatestBlocks[0]);
      blobClient.createBlockFromText(blockList.LatestBlocks[1], container, blob, blockContent2, blockContent2.length, function (error, response) {
        if (error) {
          console.log(error);
        } else {
          console.log('Uploaded the block whose ID is %s', blockList.LatestBlocks[1]);
          blobClient.createBlockFromText(blockList.LatestBlocks[2], container, blob, blockContent3, blockContent3.length, function (error, response) {
            if (error) {
              console.log(error);
            } else {
              console.log('Uploaded the block whose ID is %s', blockList.LatestBlocks[2]);
              blobClient.commitBlocks(container, blob, blockList, function (error, result, response) {
                if (error) {
                  console.log(error);
                } else {
                  console.log('Committed the blob %s', blob);
                  createSnapshot();
                }
              });
            }
...
function createSnapshot() {
  ...
  blobClient.createBlobSnapshot(container, blob, function (error, result, response) {
    if (error) {
      console.log(error);
    } else {
      console.log('Created a snapshot for the blob %s', blob);
      createBlob(result);
    }
  });
}
...
function createBlob(snapshot) {
  ...
  blobClient.createBlockFromText(blockId2, container, blob, blockContentAlternative2, blockContentAlternative2.length, function (error, response) {
    if (error) {
      console.log(error);
    } else {
      console.log('Uploaded the block whose ID is %s', blockId2);
      var blockList = {
        LatestBlocks: [blockId1, blockId2, blockId3]
      };
      blobClient.commitBlocks(container, blob, blockList, function (error, result, response) {
...
function promoteSnapshot(snapshot) {
  ...
  blobClient.createBlockBlobFromText(container, blob, 'promoted', { snapshotId: snapshot }, function (error, result, response) {
    if (error) {
      console.log(error);
    } else {
      console.log('Promoted the snapshot %s', snapshot);
...
function deleteSnapshot(snapshot) {
  ...
  blobClient.deleteBlob(container, blob, { snapshotId: snapshot }, function (error, result, response) {
    if (error) {
      ...
      console.log('Deleted the snapshot %s', snapshot);
      listOnlySnapshots();
    }
  });
};
...
function listOnlySnapshots() {
  ...
  blobClient.listBlobsSegmented(container, { prefix: blob, include: 'snapshots' }, function (error, result, response) {
    if (error) {
      ...
      console.log('Listing snapshots for the blob %s', blob);
      result.entries.forEach(function (blobResult) {
        if (blobResult.snapshot) {
          console.log('  Snapshot: %s', blobResult.snapshot);
        }
      });
    }
  });
